...
# from ome_zarr.reader import read_image
...
class OmeZarrReader(Reader):
    ...
    def _get_image_data(
        ...
        dask_data = dask_data.transpose(transpose_indices)
        ...
        return dask_data[retrieve_indices].compute(scheduler="synchronous")

    def _get_dims_for_scene(self) -> List[str]:
        # Get / guess dims
    ...
    def _create_dask_array(
        ...
        for np_index, _ in np.ndenumerate(lazy_arrays):
            ...
            lazy_arrays[np_index] = da.from_delayed(
                delayed(OmeZarrReader._get_image_data)(
                    fs=self._fs,
                    path=self._path,
